...
document.querySelectorAll('.blood-info').forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.blood-info').forEach(l => l.classList.remove('active-link'));
  
      // Add it to the clicked one
      e.target.classList.add('active-link');
  
      // Switch the content section
...
// Optional: Reset animation on button click (add a button if desired)
...
// Uncomment and add a button to trigger this if needed
// document.querySelector('#resetButton').addEventListener('click', resetAnimation);
